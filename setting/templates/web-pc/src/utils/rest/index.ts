...
export class Rest {
  ...
  private getPath() {
    const pathArray = [this.parentUrl, ...this.urls].filter(
      (path: string) => {
        return path.trim();
      }
    );
  ...
  setRequestConfig(method: Method, config?: AxiosRequestConfig, data?: any):AxiosRequestConfig {
    const params = this.requestParams;
    const {requestBody} = this;
    const base:AxiosRequestConfig = { method, params, data: data || requestBody };
    return {
      ...restConfig,
      ...config,
      ...base
