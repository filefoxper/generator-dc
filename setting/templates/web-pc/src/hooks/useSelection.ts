...
export class Selection<T> {
  ...
  update(range: Array<T>, selected?: Array<T>) {
    this.range = range;
    this.selection = computeSelected(
      this.range,
  ...
  toggle(selection: Array<T>) {
    ...
    if (!key) {
      ...
      this.selection = this.selection
        .filter((d) => !toggleSet.has(d))
        .concat(padding);
    } else {
      const toggleKeySet = new Set(selection.map((d) => d[key]));
      const selectionKeySet = new Set(this.selection.map((d) => d[key]));
      const paddingByKey = selection.filter(
        (s) => !selectionKeySet.has(s[key])
      );
      this.selection = this.selection
        .filter((d) => !toggleKeySet.has(d[key]))
        .concat(paddingByKey);
    }
    this.selection = computeSelected(this.range, this.selection, this.option);
    return this;
...
export function useSelection<T>(
  ...
  const utils = useMemo<SelectionMethods<T>>((): SelectionMethods<T> => {
    return {
      update(r: Array<T>, s?: Array<T>) {
        const { current } = selectionRef.current.update(r, s);
        setSelection(current);
      },
      select(s: Array<T>) {
        const { current } = selectionRef.current.select(s);
        setSelection(current);
      },
      unSelect(s: Array<T>) {
        const { current } = selectionRef.current.unSelect(s);
        setSelection(current);
      },
      selectAll() {
      ...
      toggle(s: Array<T>) {
        const { current } = selectionRef.current.toggle(s);
        setSelection(current);
      },
      toggleAll() {
  ...
  const selectionStatus = useMemo<SelectionStatus>(() => {
    const { current, range:allRange } = selectionRef.current;
    const currentSize = current.length;
    const rangeSize = allRange.length;
